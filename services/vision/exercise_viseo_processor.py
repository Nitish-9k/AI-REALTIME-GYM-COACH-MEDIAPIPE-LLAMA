...
class VideoProcessorClass(VideoProcessorBase):
    ...
    def _draw_skeleton(self, img, landmarks):
        ...
        # draw points first
        for lm in landmarks:
            if getattr(lm, "visibility", 0) > 0.7:
                cv2.circle(
                    img,
        ...
        for start, end in POSE_CONNECTIONS:
            ...
            if getattr(p1, "visibility", 0) > 0.7 and getattr(p2, "visibility", 0) > 0.7:
                cv2.line(
                    img,
    ...
    # def _draw_skelton(self,img,landmarks):
    #     h,w=img.shape[:2]
    ...
    def _draw_overlays(self,img,metrics,ex_type):
        ...
        if ex_type =="Biceps Curls(Dumbell)":
            self._draw_curl_overlays(img,metrics)
        if ex_type =="Shoulder Press":
    ...
    def recv(self, frame):
        ...
        # FIX 1: correct timestamp
        self._frame_timestamps_ms = int(time.monotonic() * 1000)
